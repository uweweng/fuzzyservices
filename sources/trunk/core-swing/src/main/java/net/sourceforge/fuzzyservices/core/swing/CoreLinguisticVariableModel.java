...
public class CoreLinguisticVariableModel extends AbstractLinguisticVariableModel {
    ...
    public CoreLinguisticVariableModel(LinguisticVariable linguisticVariable) {
        ...
        if (this.linguisticVariable != null) {
            for (Iterator<String> it = linguisticVariable.getNames();
                    it.hasNext();) {
                String name = it.next();
    ...
    public MembershipFunctionModel getMembershipFunctionOfLinguisticTermAt(
        int index) {
        return ((linguisticTermNames != null) && (linguisticVariable != null))
        ? new CoreMembershipModel(linguisticVariable.getFuzzySet(
                linguisticTermNames.get(index))) : null;
    }

    @Override
    public String getNameOfLinguisticTermAt(int index) {
        return (linguisticTermNames != null) ? linguisticTermNames.get(index)
                                             : null;
    }
}
