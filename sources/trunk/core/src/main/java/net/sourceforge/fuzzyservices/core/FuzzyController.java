...
public final class FuzzyController {
    ...
    public static FactBase performApproximateReasoning(RuleBase aRuleBase,
            FactBase aFactBase, LinguisticVariable[] linguisticVariables) {
        ...
        for (int i = 0; i < linguisticVariables.length; i++) {
            ...
            lv.put(linguisticVariable.getName(), linguisticVariable);
        }
                ...
                if (ruleResultFb != null) {
                    ...
                    for (Iterator<Fact> itFacts = ruleResultFb.iterator(); itFacts.hasNext();) {
                        ...
                        if (existFact != null) {
                            ...
                            FuzzySet accResult = aRuleBase.getAccumulationOperator().combine(existFact.get(),
                                    resultFact.get());
     ...
     */
    protected static FactBase applyTo(final FactBase fb, final Rule rule, HashMap<String, LinguisticVariable> lv) {
        ...
        if (rule.getCertainty() > 0.0f) {
            ...
            totalCompatibilityMeasure = ((rule.getAggregationOperator().isValidTNorm() == true) ? 1.0f
                    : 0.0f);
            ...
            for (Iterator<Antecedent> it = rule.getAntecedents(); it.hasNext();) {
                ...
                if (fact != null) {
                    ...
                    if (lingVarAnte != null) {
                        ...
                        totalCompatibilityMeasure = rule.getAggregationOperator().compute(totalCompatibilityMeasure,
                                anteCompatibilityMeasure);
                    }
                }
                // Applying the degree of compatibility to certainty
                totalCompatibilityMeasure = rule.getCertaintyOperator().compute(totalCompatibilityMeasure,
                        rule.getCertainty());
            }
                    ...
                    if (lingVarCon != null) {
                        ...
                        if (fsetLingTerm != null) {
                            ...
                            fsetComp.set(fsetLingTerm.getMaxDefinedX(),
                                    totalCompatibilityMeasure);
                            fsetInferenceResult = rule.getInferenceOperator().combine(fsetComp,
                                    fsetLingTerm);
                            // Adding inference result to fact base.
                            resultFb.put(new Fact(lingVarCon,
                                    fsetInferenceResult));
                        }
                    }
                }
