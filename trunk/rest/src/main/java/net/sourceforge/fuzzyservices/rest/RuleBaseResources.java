...
import net.sourceforge.fuzzyservices.beans.dao.RuleBaseDao;

/**
 * This class represents fuzzy set beans as resources hosted at the URI path 
 * "/rule_bases". For instance,
 * 
 * http://localhost:8080/net/sourceforge/fuzzyservices/rule_bases/4711
 * curl -i http://localhost:8080/net/sourceforge/fuzzyservices/rule_bases/4711 -H ’Accept: text/plain’
 *
 * @see RuleBaseResource
 * 
 * @version 1.0
 * @author Uwe Weng
 */
@Path("/rule_bases")
public class RuleBaseResources {
     ...
     */
    private DaoI<RuleBase, Integer> dao = new RuleBaseDao();
    /**
    ...
    public Response getById(@PathParam("id") int id) {
        RuleBase bean = dao.findById(id);
        if (bean == null) {
            return Response.status(404).build();
        }
        return Response.ok(new RuleBaseResource(bean)).build();
    }
     ...
     */
    @POST
    public Response create(RuleBaseResource newResource) {
     ...
     */
    @PUT
    public Response put(RuleBaseResource resource) {
        RuleBaseResource response = new RuleBaseResource(dao.update(resource.getBean()));
     ...
     */
    @DELETE
    public void delete(RuleBaseResource resource) {
