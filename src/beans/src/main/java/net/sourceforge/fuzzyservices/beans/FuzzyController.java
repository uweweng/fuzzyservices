...
public class FuzzyController implements FuzzyControllerI, Serializable {
    ...
    public FactBase performApproximateReasoning(RuleBase aRuleBase, FactBase aFactBase, LinguisticVariable[] linguisticVariables) {
        ...
        HashMap<String, LinguisticVariable> lv = new HashMap<>();
        ...
        HashMap<String, Fact> totalResultFacts = new HashMap<>(); // the inference result as hash map with die name of the linguistic variable as key
        FactBase ruleResultFb; // the inference result of one rule

        // Iterating all rules of the rule base
        Rule[] rules = aRuleBase.getRules();
        for (int itRules = 0; itRules < rules.length; itRules++) {
            Rule actRule = rules[itRules];
            ...
            if (actRule != null) {
                try {
                    ...
                    if (ruleResultFb != null) {
                        ...
                        Fact[] facts = ruleResultFb.getFacts();
                        for (int itFacts = 0; itFacts < facts.length; itFacts++) {
                            resultFact = facts[itFacts];
                            existFact = totalResultFacts.put(resultFact.getLinguisticVariableName(), resultFact);

                            if (existFact != null) {
                                // Es wurde bereits ein Ergebnis zu dieser ling. Variable ermittelt.
                                // Deshalb ist eine Akkumulation notwendig.
                                FuzzySet accResult =
                                        aRuleBase.getAccumulationOperator().combine(existFact.getValue(),
                                        resultFact.getValue());
                                resultFact.setValue(accResult);
                                // Akkumuliertes Ergebnis in die zurueckzugebende Faktenbasis einfuegen.
                                totalResultFacts.put(resultFact.getLinguisticVariableName(), resultFact);
                            }
                        }
                    }
        ...
        FactBase totalResultFb = new FactBase();
        Collection<Fact> cFacts = totalResultFacts.values();
        totalResultFb.setFacts(cFacts.toArray(new Fact[cFacts.size()]));
     ...
     */
    protected FactBase applyTo(final FactBase fb, final Rule rule, HashMap<String, LinguisticVariable> lv) throws PropertyVetoException {
        HashMap<String, Fact> resultFacts = new HashMap<>(); // the result as hash map with die name of the linguistic variable as key
        ...
        if (rule.getCertainty() > 0.0f) {
            ...
            // Putting facts into hashmap for identifying by name
            HashMap<String, Fact> facts = new HashMap<>();
            Fact[] factsOfFactBase = fb.getFacts();
            for (int i = 0; i < factsOfFactBase.length; i++) {
                facts.put(factsOfFactBase[i].getLinguisticVariableName(), factsOfFactBase[i]);
            }
            
            // At first, we are iterating all antecedents
            // in order to calculate compatibility.
            Antecedent[] antecedents = rule.getAntecedents();
            for (int itAnte = 0; itAnte < antecedents.length; itAnte++) {
                Antecedent ante = antecedents[itAnte];
                fact = facts.get(ante.getLinguisticVariableName());
                ...
                if (fact != null) {
                    ...
                    if (lingVarAnte != null) {
                        fsetLingTerm = lingVarAnte.getLinguisticTerm(ante.getLinguisticTermName()).getFuzzySet();
                        fsetFact = fact.getValue();
                        ...
                        totalCompatibilityMeasure = OperatorUtils.convert(rule.getAggregationOperator()).compute(totalCompatibilityMeasure, anteCompatibilityMeasure);
                    }
                }

                // Applying the degree of compatibility to certainty
                totalCompatibilityMeasure =
                        OperatorUtils.convert(rule.getCertaintyOperator()).compute(totalCompatibilityMeasure,
                        rule.getCertainty());
            }
            ...
            if (totalCompatibilityMeasure > 0.0f) {
                ...
                MembershipFunction mfComp;
                FuzzySet fsetComp; // the total degree of compatibility as fuzzy set (!!!)

                Consequent[] consequents = rule.getConsequents();
                for (int itCons = 0; itCons > consequents.length; itCons++) {
                    Consequent con = consequents[itCons];

                    LinguisticVariable lingVarCon = lv.get(con.getLinguisticVariableName());

                    if (lingVarCon != null) {
                        fsetLingTerm = lingVarCon.getLinguisticTerm(con.getLinguisticTermName()).getFuzzySet();

                        if ((fsetLingTerm != null) && (fsetLingTerm.getMembershipFunction() != null)) {
                            // Let's inference
                            // We are transforming the total degree of compatibility to a fuzzy set.
                            mfComp = new MembershipFunction();
                            mfComp.setDegreeOfMembership(fsetLingTerm.getMembershipFunction().getMinDefinedX(),
                                    totalCompatibilityMeasure);
                            mfComp.setDegreeOfMembership(fsetLingTerm.getMembershipFunction().getMaxDefinedX(),
                                    totalCompatibilityMeasure);
                            
                            fsetComp = new FuzzySet(mfComp);
                            fsetInferenceResult = rule.getInferenceOperator().combine(fsetComp, fsetLingTerm);
                            // Adding inference result to fact base.
                            resultFacts.put(lingVarCon.getName(), new Fact(lingVarCon.getName(), fsetInferenceResult));
                        }
                    }
                }
            }
        ...
        FactBase resultFb = new FactBase();
        Collection<Fact> cFacts = resultFacts.values();
        resultFb.setFacts(cFacts.toArray(new Fact[cFacts.size()]));
        return resultFb;
    }
}
